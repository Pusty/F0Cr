	...
	public void read(DataInputStream s) throws Exception {
		...
		for(int i=0;i<poolSize-1;i++) {
			...
			if(tag == UTF_8) //READ UTF_8 STRING
				...
				poolContent[i+1] = Integer.valueOf(s.readInt());
			else if(tag == FLOAT)
				poolContent[i+1] = Float.valueOf(s.readFloat());
			else if(tag == LONG) {
				poolContent[i+1] = Long.valueOf(s.readLong());
				i++;
			}else if(tag == DOUBLE) {
				poolContent[i+1] = Double.valueOf(s.readDouble());
				i++;
