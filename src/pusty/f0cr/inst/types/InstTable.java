...
public class InstTable extends Instruction {
	...
	public int getBranchDefaultInstructionPos() {
		return (reader.getPosition(this,getBranchDefault()));
	}
	
	public int[] getBranchListInstructionPos() {
		int[] a = new int[getBranchList().length];
		for(int i=0;i<a.length;i++) a[i] = (reader.getPosition(this,getBranchList()[i]));
		return a;
	}
