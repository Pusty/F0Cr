...
public class InstMath extends Instruction {
	...
	public InstMath(InstructionReader reader, byte inst, byte[] data) {
		...
		if(!isInst(inst)  && (inst&0xFF) != Opcodes.WIDE) {System.err.println("Error: Created Math with OpCode: "+inst+" => " +Opcodes.getName(inst));}
	...
	public int getLocalVariable() {
		if(operation==MATH_INC) {
			if((inst&0xFF) == Opcodes.WIDE) {
				return (InstructionHandler.readShort(data, 1));
			}else {
	...
	public int getConstantValue() {
		if(operation==MATH_INC) {
			if((inst&0xFF) == Opcodes.WIDE) {
				return (int)((short)(InstructionHandler.readShort(data, 3)));
			}else {
				return (int)((byte)(InstructionHandler.readByte(data, 1))); 
			}
	...
	public Class<?> setType() {
		...
		case Opcodes.WIDE:
			...
			case Opcodes.IINC: return int.class;
			default: return null;
			}
		default: return null;
	}
	...
	public int setOperation() {
		switch(inst&0xFF) {
			...
			case Opcodes.WIDE:
				switch(InstructionHandler.readByte(data, 0)) {
				case Opcodes.IINC: return MATH_INC;
